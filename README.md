...
Como utilizar o `sqlmodel` no ipython:

```python

In [1]: from sqlmodel import Session, select

In [2]: from beerlog.database import engine

In [3]: from beerlog.models import Beer

In [4]: with Session(engine) as session:
   ...:     beer = Beer(name="Heineken", style="Pilsen", flavor=8, image=3, cost=7)
   ...:     session.add(beer)
   ...:     session.commit()

In [5]: with Session(engine) as session:
   ...:     sql = select(Beer)
